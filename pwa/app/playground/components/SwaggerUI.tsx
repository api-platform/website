...
import * as parseHeaders from "http-headers";
...
function phpFetch(url: URL, opts: any = {}) {
  ...
  const head = parseHeaders(parts[0]);
  const body = parts[1];
...
function SwaggerUIReact(props: any) {
  const ui = SwaggerUIConstructor(props);
  const SwaggerUIComponent = ui.getComponent("App", "root");
  return <SwaggerUIComponent />;
}

export default function SwaggerUI({ guide, ccall }: { guide: string, ccall: any }) {
