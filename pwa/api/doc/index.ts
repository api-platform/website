...
import { Lang, getHighlighter, loadTheme } from "shiki";
...
export const getHtmlFromGithubContent = async ({
  ...
  const light = await loadTheme(
    path.join(process.cwd(), "vendor/shiki/themes/github-light.json")
  );
  const dark = await loadTheme(
    path.join(process.cwd(), "vendor/shiki/themes/one-dark-pro.json")
  );

  const lightHighlighter = await getHighlighter({
    theme: light,
    paths: {
      languages: path.join(process.cwd(), "vendor/shiki/languages"),
    },
  });
  ...
  const languages = lightHighlighter.getLoadedLanguages();
  ...
  marked.use(
    markedHighlight({
      ...
      highlight(code, language) {
        ...
        return (
          lightHighlighter.codeToHtml(code, { lang }) +
          darkHighlighter.codeToHtml(code, { lang })
        );
      },
    })
  );
